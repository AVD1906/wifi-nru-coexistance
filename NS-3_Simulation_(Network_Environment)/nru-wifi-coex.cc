...
Program Title: NS-3 (Network Simulator) for NRU Wifi Coexistence

NOTE: To be placed in ns-3.xx/scratch
*/
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    double totalDelayNru = 0.0;
    double totalDelayWifi = 0.0;
    uint32_t rxPacketsNru = 0;
    uint32_t rxPacketsWifi = 0;
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
    {
        ...
        if (t.destinationPort == port) { 
            totalDelayNru += i->second.delaySum.GetSeconds();
            rxPacketsNru += i->second.rxPackets;
        } 
        else if (t.destinationPort == port + 1) {
            totalDelayWifi += i->second.delaySum.GetSeconds();
            rxPacketsWifi += i->second.rxPackets;
        }
    }

    double actualNruDly = (rxPacketsNru > 0) ? (totalDelayNru / rxPacketsNru) * 1000.0 : 0.0;
    double actualWifiDly = (rxPacketsWifi > 0) ? (totalDelayWifi / rxPacketsWifi) * 1000.0 : 0.0;
    */
